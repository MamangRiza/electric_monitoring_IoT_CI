...
class Config extends AUTH_Controller {
	...
	public function index() {
		...
		$data['page'] 		= "config";
		$data['judul'] 		= "Data Config";
		$data['deskripsi'] 	= "Manage Data Config";

		$data['modal_tambah_kota'] = show_my_modal('modals/modal_tambah_config', 'tambah-config', $data);

		$this->template->views('config/home', $data);
	}

	public function tampil() {
		$data['dataConfig'] = $this->M_config->select_all();
		$this->load->view('config/list_data', $data);
	}
	...
	public function prosesTambah() {
		...
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_config->insert($data);
	...
	public function update() {
		...
		$id 				= trim($_POST['id']);
		$data['dataConfig'] 	= $this->M_config->select_by_id($id);

		echo show_my_modal('modals/modal_update_config', 'update-config', $data);
	}
	...
	public function prosesUpdate() {
		...
		$data['dirubah'] = "ya";
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_config->update($data);

			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data Config Berhasil diupdate', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data Config Gagal diupdate', '20px');
			}
		} else {
			$out['status'] = 'form';
			$out['msg'] = show_err_msg(validation_errors());
	...
	public function delete() {
		$id = $_POST['id'];
		$result = $this->M_config->delete($id);
	...
	public function detail() {
		...
		$id 				= trim($_POST['id']);
		$data['kota'] = $this->M_config->select_by_id($id);
		$data['jumlahKota'] = $this->M_config->total_rows();
		$data['dataKota'] = $this->M_config->select_by_pegawai($id);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				foreach ($sheetData as $key => $value) {
					if ($key != 1) {
						$check = $this->M_config->check_nama($value['B']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_config->insert_batch($resultData);
					if ($result > 0) {
